...
def _get_metric_value(metrics: Dict[str, Any], key: str) -> Optional[float]:
    """Returns a metric value by key with a case-insensitive fallback."""
    if key in metrics:
        return metrics[key]
    if key:
        key_lower = key.lower()
...
async def evaluate_and_store(
    ...
    if fitness_val is None:
        logger.warning(
            f"Fitness key '{fitness_key}' not found in eval metrics; defaulting fitness to 0."
        )
        fitness_val = 0
    child_sol.fitness = fitness_val
...
def _initialize_new(
    ...
    if init_sol.returncode == 0:
        ...
        if fitness_val is None:
            logger.warning(
                f"Fitness key '{fitness_key}' not found in init eval metrics; defaulting fitness to 0."
            )
            fitness_val = 0
