...
def S_TT(ell, params):
    ...
    # New feature: Torsion resonance at low ell for bounce fluctuations
    bump_center = 5.0
    bump_width = 1.5
    fourier_sum += 0.035 * math.cos(math.pi * (ell_float - bump_center) / bump_width)
    fourier_sum += 0.020 * math.cos(2.0 * math.pi * (ell_float - bump_center) / bump_width)
    # Secondary resonance at ell=14
    res_center = 14.0
    res_width = 2.0
    fourier_sum += -0.025 * math.cos(math.pi * (ell_float - res_center) / res_width)
